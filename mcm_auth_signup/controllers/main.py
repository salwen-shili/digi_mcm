...
class AuthSignupHome(AuthSignupHome):
    ...
    def do_signup(self, qcontext):
        ...
        if 'passerelle' in qcontext : #check if passerelle in qcontext
            values['street'] = values['nom_voie'] #pass value of nom_voie to street field
...
class Home(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if partner:
            if 'passerelle' in kw: #check if passerelle passed in parametre of login
                product_id = request.env['product.product'].sudo().search(
                            [('default_code', "=", "passerelle-taxi"), ('company_id', "=", 1)], limit=1) #check product of passerelle in product product database table
                if product_id:
                    ...
                    # create sale order with formation passerelle
                    if so:
                        so.sudo().write({
                            'amount_untaxed': product_id.list_price,
                        })
                        # redirect to /felicitations
