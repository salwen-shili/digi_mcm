...
class Api(models.Model):
    ...
    redirect_uri = fields.Char(string="Calendly redirect URI",
                               required=True, default='https://www.mcm-academy.fr/')
    ...
    def _refresh_token(self):
        ...
        if calendly:
            ...
            r = requests.post(
                'https://auth.calendly.com/oauth/token', headers=headers, data=data)
            response = werkzeug.utils.unescape(r.content.decode())
            json_data = json.loads(r.text)
            print(json_data)
            if "access_token" in json_data:
                ...
                self._cr.execute(
                    """UPDATE calendly_api SET access_token = %s WHERE id=%s""", (access, calendly.id,))
                self._cr.commit()
                self._cr.execute(
                    """UPDATE calendly_api SET refresh_token = %s WHERE id=%s""", (refresh, calendly.id,))
                self._cr.commit()
                ...
                if "collection" in json_data_events:
                    ...
                    for event in collection:
                        ...
                        if 'ae1aafc2-ae62-4151-b6a6-e75d1db9c736' in event['uri']:
                            ...
                            response = werkzeug.utils.unescape(
                                r.content.decode())
                ...
                if "collection" in json_data_events:
                    ...
                    for event in collection:
                        ...
                        if 'ae1aafc2-ae62-4151-b6a6-e75d1db9c736' in str(event_type):
                            ...
                            r = requests.get(
                                str(uri)+'/invitees', headers=headers)
                            response = werkzeug.utils.unescape(
                                r.content.decode())
                                        ...
                                        if not odoo_contact:
                                            if tel:
                                                ...
                                                if not odoo_contact:
                                                    phone_number = str(
                                                        tel).replace(' ', '')
                                                    # check if aircall api send the number of client with +33
                                                    if '+33' not in str(phone_number):
                                                        phone = phone_number[0:2]
                                                        # check if aircall api send the number of client in this format (number_format: 33xxxxxxx)
                                                        if str(phone) == '33' and ' ' not in str(tel):
                                                            phone = '+' + \
                                                                str(tel)
                                                            odoo_contact = self.env["res.users"].sudo().search(
                                                                [("phone", "=", phone)], limit=1)
                                                            if not odoo_contact:
                                                                ...
                                                                odoo_contact = self.env["res.users"].sudo().search(
                                                                    [("phone", "=", phone)], limit=1)
                                                        phone = phone_number[0:2]
                                                        # check if aircall api send the number of client in this format (number_format: 33 x xx xx xx)
                                                        if str(phone) == '33' and ' ' in str(tel):
                                                            phone = '+' + \
                                                                str(tel)
                                                            odoo_contact = self.env["res.users"].sudo().search(
                                                                ['|', ("phone", "=", phone),
                                                        ...
                                                        phone = phone_number[0:2]
                                                        # check if aircall api send the number of client in this format (number_format: 07xxxxxx)
                                                        if str(phone) in ['06', '07'] and ' ' not in str(tel):
                                                            odoo_contact = self.env["res.users"].sudo().search(
                                                                [("phone", "=", str(
                                                                    tel))],
                                                                limit=1)
                                                            if not odoo_contact:
                                                                ...
                                                                odoo_contact = self.env["res.users"].sudo().search(
                                                                    [("phone", "=", phone)], limit=1)
                                                        phone = phone_number[0:2]
                                                        # check if aircall api send the number of client in this format (number_format: 07 xx xx xx)
                                                        if str(phone) in ['06', '07'] and ' ' in str(tel):
                                                            odoo_contact = self.env["res.users"].sudo().search(
                                                                ['|',
                                                                 ("phone", "=",
                                                                  str(tel)),
