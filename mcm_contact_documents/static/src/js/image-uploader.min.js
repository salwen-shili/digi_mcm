 ...
 * Copyright (c) 2019 Christian Bayer; Licensed MIT */
    ...
    $.fn.imageUploader = function (options) {
        ...
        let defaults = {
            ...
            label: 'Cliquez ici pour charger votre document',
            extensions: ['.jpg', '.jpeg', '.png','.pdf','.PNG','.JPG','.JPEG','.PDF'],
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                // Create the img tag
                $img = $('<img src="/mcm_contact_documents/static/src/file.png"/>',{src: src}).appendTo($container),
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                alert(`le fichier "${file.name}" ne correspond pas aux extensions acceptées: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                alert(`le fichier "${file.name}" ne correspond pas aux types acceptés: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                alert(`le fichier "${file.name}" dépasse la taille maximale de ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                if (plugin.settings.imagesInputName=='changed_file'){
                alert(`Vous pouvez pas télécharger plus que ${plugin.settings.maxFiles} fichiers`);
                } else {
                alert(`Vous pouvez pas télécharger plus que ${plugin.settings.maxFiles} fichiers en ${plugin.settings.dropzoneName} `); }
...
//            maxFiles: 2
        ...
        $('.input-images-1').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Piéce d'identité",
            imagesInputName: 'identity',
            preloadedInputName: 'old',
        ...
        $('.input-images-2').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Attestation d'hebergeur",
            imagesInputName: 'attestation',
            preloadedInputName: 'old',
            maxSize: 24 * 1024 * 1024,
            maxFiles: 1
        });
		$('.input-images-3').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Permis de conduire",
            imagesInputName: 'permis',
            preloadedInputName: 'old',
            maxSize: 24 * 1024 * 1024,
            maxFiles: 2
        });
...
$('.input-images-5').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Justificatif de domicile",
            imagesInputName: 'domicile',
            preloadedInputName: 'old',
            maxSize: 24 * 1024 * 1024,
            maxFiles: 1
        });
$('.input-images-6').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Carte identité d'hébergeur",
            imagesInputName: 'hebergeur_identity',
            preloadedInputName: 'old',
            maxSize: 24 * 1024 * 1024,
            maxFiles: 2
        });
    });
$('#identity').imageUploader({
            preloaded: preloaded,
            dropzoneName: "Carte identité Recto",
            imagesInputName: 'hebergeur_identity',
            preloadedInputName: 'old',
            maxSize: 24 * 1024 * 1024,
            maxFiles: 1
...
jQuery(document).ready(function($){
    ...
    $(document).keyup(function(event){
        if(event.which=='27'){
            $('.cd-popup5').removeClass('is-visible');
        }
    });
    ...
    $(document).keyup(function(event){
        if(event.which=='27'){
            $('.cd-popup7').removeClass('is-visible');
        }
    });
    ...
    $(document).keyup(function(event){
        if(event.which=='27'){
            $('.cd-popup8').removeClass('is-visible');
        }
    });
    ...
    $(document).keyup(function(event){
        if(event.which=='27'){
            $('.cd-popup9').removeClass('is-visible');
        }
    });
    ...
    $(document).keyup(function(event){
        if(event.which=='27'){
            $('.cd-popup6').removeClass('is-visible');
        }
    });
});