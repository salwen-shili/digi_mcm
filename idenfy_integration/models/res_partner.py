...
class ResPartner(models.Model):
    ...
    def _get_is_idenfy_approved(self):
        for rec in self:
            ...
            if rec.idenfy_document_data_id.status == 'APPROVED' and rec.idenfy_dl_data_id.status == 'APPROVED':
    ...
    def check_status(self,website):
        for rec in self:
            ...
            if doc_check_status and doc_check_status.get('status') in ['ACTIVE'] or check_status and check_status.get('status') in ['ACTIVE']:
                return True
        return False
    ...
    def fetch_document_details_from_idenfy(self,website):
        ...
        for rec in self:
            ...
            rec.idenfy_document_data_id.write({'res_data': dl_response})
            rec.write({
