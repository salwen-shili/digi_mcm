...
class MailMail(models.Model):
    ...
    cc_partner_ids=fields.Many2many('res.partner','mail_mail_res_p_rel','mail_mail_id',  string=' CC(Partners)',context={'active_test': False})
    bcc_partner_ids=fields.Many2many('res.partner', 'mail_mail_part_rel','mail_mail_id', string='BCC(Partners)',context={'active_test': False})
    ...
    def create(self, values):
        ...
        record=self.env["account.invoice.send"].sudo().search([],order="id desc", limit=1)
       
        model = False
        if values.get("mail_message_id",False):
            model = self.env["mail.message"].browse(values.get("mail_message_id",False)).model
