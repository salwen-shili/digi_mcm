             ...
             */
              _onDeleteClick: function(ev){
               if(ks_session.is_admin && ks_session.ks_admin_delete_access){
                var ks_result = confirm("Etes-vous sûr de vouloir supprimer définitivement ce message ?");
               } else {
                      var ks_result = false;
               }
               var ks_self = this;
...
//                       }
//                });
                    if(ks_session.is_admin && ks_session.ks_admin_delete_access && ks_result){
                   var ks_notif_div = document.createElement("div");
                   ks_notif_div.id = "snackbar";

                   var ks_textnode = document.createTextNode("Le message a bien été supprimé.");
