...
class IrHttp(models.AbstractModel):
            ...
            if '/web/image' not in req_page and '/web/content' not in req_page:
                try:
                    ...
                    if redirect and useServerRewrites:
                        ...
                        if redirect.rewrite_val == 'product.template' and website_id.use_category_url:
                            currentPage = request.httprequest.path
                            currentPage = currentPage.split('/')
                            currentPage = '/'.join(currentPage[:-1])
                            if currentPage:
                                url_to = currentPage+url_to
    ...
    def _serve_url_to_redirect(cls, req_page, redirectObj):
        try:
            ...
            redirectObjs = redirectObj.search(domain, limit=1)
            if not redirectObjs:
                resObjs = request.env['ir.translation'].sudo().search(transDomain, limit=1)
                if resObjs:
                    urlKey = "/" + resObjs.src
