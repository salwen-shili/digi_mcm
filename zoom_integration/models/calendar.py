...
class Meeting(models.Model):
    ...
    def jwt_api_access(self):
        ...
        if company_rec.api_key and company_rec.api_secret:
            ...
            print("self.env.user.zoom_login_email", self.env.user.zoom_login_email)
                    ...
                    for all_user in all_users:
                        if all_user['email'] == self.env.user.zoom_login_email:
                            if all_user['status'] == 'active':
                                ...
                                print("self.env.user.zoom_login_user_id",self.env.user.zoom_login_user_id)


                                self.env.user.zoom_user_timezone = all_user['timezone']
                                print("self.env.user.zoom_user_timezone",self.env.user.zoom_user_timezone)
                                if not all_user['timezone']:
                                    raise UserError(_('Please set zoom user timezone'))
                                break;
                            else:
                                raise UserError(_('Invalid zoom user 1'))

                    if not zoom_user:
                        raise UserError(_('Invalid zoom user 2'))
                else:
                    raise UserError(_('Zoom API Exception'))

            else:
                raise UserError(_('Invalid zoom user 3'))
