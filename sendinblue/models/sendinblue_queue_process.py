...
class SendINblueQueueProcess(models.Model):
    ...
    processed_contact_ids = fields.Many2many('mailing.contact', 'mailing_contact_sendinblue_queue_rel', 'queue_id',
                                             'contact_id', string='Processed Contacts')  # v13
    ...
    def sendinblue_create_queue(self):
        ...
        mass_malling_ids = mass_malling_obj.search(
            [('state', 'in', ['in_queue', 'done', 'sending']), ('sendinblue_id', '!=', False)])
    ...
    def process_pending_to_export_contact(self):
        export_contact_list = self or self.search(
            [('operation', '=', 'pending_to_export_contact'), ('state', 'in', ['in_queue']), ('list_id', '!=', False)])
    ...
    def process_fetch_member(self):
        queue_records = self or self.search(
            [('operation', '=', 'fetch_member'), ('state', 'in', ['in_queue']), ('list_id', '!=', False)])
    ...
    def process_campaign_activity_report(self):
        ...
        queue_records = self or self.search(
            [('operation', '=', 'campaign_activity_report'), ('state', 'in', ['in_queue']),
             ('campaign_id', '!=', False)])
        for rec in queue_records:
