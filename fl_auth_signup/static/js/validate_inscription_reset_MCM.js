$(document).ready(function () {
  if (document.querySelectorAll('button.rkmd-btn')) {
    if (document.querySelectorAll('button.rkmd-btn').length == 2) {
      document.querySelectorAll('button.rkmd-btn')[1].remove();
    }
  }
  ...
  function checkPhone(prop, value) {
    if (value.length > 0) {
      ...
      else return `Votre numéro de téléphone doit commencer par 06 ou 07 suivie par 8 chiffres`;
  ...
  function checkPassword(value) {
    if (value.length > 0) {
      ...
      if (!value.match(pattern)) return 'Votre mot de passe doit respecter les 4 exigences ci-dessous';
      else return false;
      // else {
      //   const pattern =
      //     /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!/+/='":;(){}[|@$ %^&*-]).{8,}$/;
...
function checkStrength(password) {
  ...
  if (strength == 1) {
    ...
    passwordStrength.classList.remove('progress-bar-danger');
    passwordStrength.classList.add('progress-bar-warning');
    passwordStrength.style = 'width: 65%';
    nivLevel.innerHTML = 'moyenne';
    nivLevel.classList.add('niv-level-2');
    nivLevel.classList.remove('niv-level-1');
    nivLevel.classList.remove('niv-level-3');
